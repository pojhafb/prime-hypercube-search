...
consistent with convergence toward 1.
We conjecture $\rho_m(a) \to S_{\mathrm{xor}}(a)$ as $m \to \infty$.
\end{abstract}

\noindent\textbf{MSC 2020:} 11N05 (primary); 11A41, 11N35, 06E30, 05A05 (secondary).

\smallskip
\noindent\textbf{Keywords:} prime pairs, XOR autocorrelation, odd binary hypercube,
...
at $m \in \{16, 18, 20, 22\}$ using the WHT, and compute $S_{\mathrm{xor}}(a)$
by numerical estimation of the local factors over odd integers in $[1, 2^{18})$.
...
22 & 7546 & $0.760^\dagger$ & 1.002 & 0.05 \\
\bottomrule
\end{tabular}
\end{table}

${}^\dagger$At $m=22$, masks involving bits $18$--$21$ lie outside the estimator
range $[1,2^{18})$; within that range their XOR degenerates to addition,
distorting numerical local factors and reducing global Pearson~$r$.
Weights~1 and~2 use the closed-form formulas of
Theorems~\ref{thm:weight1}--\ref{thm:weight2} and are unaffected.

Per-weight convergence of $\rho/S_{\mathrm{xor}}$ toward~1:

\begin{table}[h]
\centering
\caption{Mean $\rho_m/S_{\mathrm{xor}}$ by mask weight across dimensions
(weights~1--2 use exact analytical formulas;
weights~3--4 at $m=22$ are affected by the estimator-range limitation).}
\label{tab:byweight}
\begin{tabular}{lrrr}
\toprule
...
The weight-1 ratio at $m=22$ ($0.9967$) is closer to~1 than at $m=20$ ($0.9946$),
consistent with convergence to $S_{\mathrm{xor}}(2^j) = C_2 \approx 0.66016$
(Corollary~\ref{cor:C2}).
Weight-2 similarly converges: $0.9990$ at $m=22$ vs.\ $0.9995$ at $m=20$.
No persistent offset is visible for weights~1--2.
...
(i)~Does $\rho_m(a)/S_{\mathrm{xor}}(a) \to 1$ exactly?
At $m=22$ the weight-1 and weight-2 ratios are $0.9967$ and $0.9990$,
with no sign of a persistent offset, but the convergence rate is slow.
(ii)~Do weight-3 local factors admit a closed form in terms of the
three pairwise bit-differences, or are there genuine three-body interactions?
(iii)~Can Conjecture~\ref{conj:main} be proved for any infinite family of masks?

\paragraph{Sieve structure vs.\ search.}
A natural question is whether the ordering of flip masks by $S_{\mathrm{xor}}$
improves prime search.
Direct comparison (10\,000 samples, $m \in \{32, 40, 48\}$) shows that
SieveAwarePolicy --- which ranks flip sets by $S_{\mathrm{xor}}$ descending
within each Hamming radius --- is marginally \emph{slower} than
LowBitFirst ($-1$--$2\%$ in mean checks).
The reason is conceptual: $S_{\mathrm{xor}}(a)$ measures the density of
XOR pairs where \emph{both} endpoints are prime; the search problem requires only
the \emph{target} $x \oplus a$ to be prime given an arbitrary starting point~$x$.
These are different events, and $S_{\mathrm{xor}}$ is not predictive for the latter.
